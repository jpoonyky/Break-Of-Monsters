...
float	EnvParam = 0.9f;	//	映りこみの割合

float AdjustRain;	//	流量
texture RainMap;	//	雨法線テクスチャ
sampler RainSamp = sampler_state
{
    Texture = <RainMap>;
    MinFilter = LINEAR;
    MagFilter = LINEAR;
...
//		アウトライン頂点シェーダー	
//------------------------------------------------------
float3 outcolor = { 0.2f, 0.2f, 1.0f };
...
float4 Environment2( float3 normal, float2 uuv )
{
	float4	color;
	...
	color = tex2D( EnvironmentSamp, uv + uuv );
	return color;
}
...
float4 PS_Level4SP1( VS_OUTPUT In) : COLOR
{   
	float4	OUT;
	float2 Tex = In.Tex;
	...
	float2 RainUV = Tex;
	RainUV.y += AdjustRain;
	float3 Rain = tex2D( RainSamp, RainUV ).xyz * 2.0f - 1.0f;
	RainUV.y += AdjustRain;
	RainUV.x = -RainUV.x;
	Rain += tex2D( RainSamp, RainUV ).xyz * 2.0f - 1.0f;
	Rain = normalize( Rain );
	
	//	雨法線→元テクスチャの揺らぎ
	OUT2 = In.Color * tex2D( DecaleSamp, Tex-Rain.xy*0.04f );
	
	//	反射ベクトルの計算
	float3 R2 = reflect( -E, N+Rain );
	R2 = normalize(R2);
	//	雨環境マップの合成
	OUT2 *= Environment(R2);
	float rate = DirLight2(In.vLight, Rain);
	//	元画像との合成	
...
//		ピクセルシェーダー	
//------------------------------------------------------
float4 PS_Level4SP2( VS_OUTPUT In) : COLOR
{   
	float4	OUT;
	float2 Tex = In.Tex;
	...
	//	パララックスディスプレースメント
	float h = tex2D( HeightSamp, Tex -Rain.xy*0.04f).r-0.5f;
	float3 E = normalize(In.vE);
	Tex.x += 0.04f * h * E.x;
	Tex.y -= 0.04f * h * E.y;
	...
	float3 R = reflect( -E, N );
	//	スペキュラマップ取得
	float4 sp_tex = tex2D( SpecularSamp, Tex-Rain.xy*0.04f );
	...
	//float3 env = (1-sp_tex.a) * tex2D( EnvironmentSamp, uv - Rain.xy*0.04f );
	//float3 env = (1-sp_tex.a) * Environment2(R,-Rain.xy*0.04f);

	//	ピクセル色決定
	OUT = In.Color * tex2D( DecaleSamp, Tex-Rain.xy*0.04f );
	OUT.rgb = (OUT.rgb + light);
...
//--------------------------------------------------
	...
	R2 = normalize(R2);
	//	雨環境マップの合成
	//OUT2 *= Environment2(R2, -Rain.xy*0.04f);
	float rate = DirLight2(In.vLight, Rain);
	//	元画像との合成	
	rate = pow(rate, 3) + 0.5f;
	OUT2 += pow( max( 0, dot(R2, In.vLight) ), 10 ) * sp_tex;
...
technique level4
{
    pass P0
    {
		...
		VertexShader = compile vs_2_0 VS_FullFX();
		PixelShader  = compile ps_3_0 PS_Level4();
    }
}
...
technique level4Sp1
{
    pass P0
    {
		...
		VertexShader = compile vs_2_0 VS_FullFX();
		PixelShader  = compile ps_3_0 PS_Level4SP1();
    }
}
...
technique level4Sp2
{
    pass P0
    {
		...
		VertexShader = compile vs_2_0 VS_FullFX();
		PixelShader  = compile ps_3_0 PS_Level4SP2();
    }
}