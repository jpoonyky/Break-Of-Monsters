...
BOOL	InitApp( HWND hWnd )
{
	...
	//	初期シーン登録
	MainFrame->ChangeScene( new sceneLoading() );
...
HWND InitWindow( HINSTANCE hInstance, int nCmdShow )
{
	...
	wc.lpszClassName = "IEX2016_Ver1.1";
	RegisterClass(&wc);

	//	ウィンドウ作成
	if( !bFullScreen ){
		AdjustWindowRect( &WindowSize, WS_OVERLAPPEDWINDOW, FALSE );
		hWnd = CreateWindow( "IEX2016_Ver1.1", AppTitle, WS_OVERLAPPEDWINDOW, 
								0, 0, WindowSize.right-WindowSize.left, WindowSize.bottom-WindowSize.top,
								NULL, NULL, hInstance, NULL);
	} else {
		hWnd = CreateWindow( "IEX2016_Ver1.1", AppTitle, WS_POPUP, 0, 0, WindowSize.right,WindowSize.bottom, NULL, NULL, hInstance, NULL);
		ShowCursor(FALSE);
	}
	if( !hWnd ) return NULL;
