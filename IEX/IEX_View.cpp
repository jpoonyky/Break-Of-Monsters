...
//		スクリーン座標変換設定
//*****************************************************************************
bool iexView::ScreenTransform(Vector3& output, const Vector3 pos, const Matrix& mat)
{
...
output.x = (float)(temp.x*WinWide + WinWide);
output.y = (float)(-temp.y*WinHeight + WinHeight);
output.z = temp.z;
...
void iexView::Activate()
{
	...
	p = Pos;
	t = Target;
	LookAtLH( matView, p, t, Vector3( 0.0f, 1.0f, 0.0f ) );
...
void iexView::Activate(Vector3 UP)
{
	//	ビュー行列設定
	Vector p, t;
	p = Pos;
	t = Target;
	LookAtLH(matView, p, t, UP);
