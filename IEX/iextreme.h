...
void	SetTransformMatrixXYZ( Matrix *Mat, float x, float y, float z);
//		ビュー変換表列
...
public:
	...
	*/
	static void RotationAxis(const  Vector3&axis, float angle, Quaternion &out)
	{
		angle /= 2;
		float a = sinf(angle);
		out.w = cosf(angle);
...
public:
	...
	//カメラ初期使用（カメラの上方向ベクトル固定）
	void Activate();
	//カメラ（カメラの上方向ベクトル引数指定可）
	void Activate(Vector3 UP);

	//スクリーン座標変換
	bool ScreenTransform(Vector3& output, const Vector3 pos)
	{
		return ScreenTransform(output, pos, matView* matProjection);
	}
	bool ScreenTransform(Vector3& output, const Vector3 pos, const Matrix& mat);
};
...
//		作成フラグ
//------------------------------------------------------
#define IEX2D_SHADOWMAP	227		// iex2DObjのコンストラクタで使用

//------------------------------------------------------
//		シャドウマップ作成クラス
//------------------------------------------------------
class TAShadowMap : public iex2DObj
{
private:
	IDirect3DSurface9* lpDepth;		// 現在
	IDirect3DSurface9* oldDepth;	// 保存用デプス
	IDirect3DSurface9* oldSurface;	// 保存用サーフェイス
public:
	TAShadowMap(u32 width, u32 height);								// テクスチャ作成
	void ShadowStart(iexView* view, Vector3 light_vec,Vector3 ppos, float dist,int znear,int zfar);		// ここでレンダーターゲットの切り替え
	void ShadowStart(iexView* view, Vector3 light_vec, float dist);											
	void ShadowEnd();		// Startでテクスチャ側に切り替えていたのを元のSurfaceに戻す			
};
...
public:
	...
	Vector3 front;
	Vector3 GetFront(){
		front.x = TransMatrix._31;
		front.y = TransMatrix._32;
		front.z = TransMatrix._33;
		front.Normalize(); 
		return front;
	}
	...
	//上方向取得
	Vector3 Upside;
	Vector3 GetUpside(){ return Upside; }

	//右方向
	Vector3 Right;
	Vector3 GetRight(){ 
		Right.x = TransMatrix._11;
		Right.y = TransMatrix._12;
		Right.z = TransMatrix._13;
		Right.Normalize();
		return Right; }
	//------------------------------------------------------
	//	初期化
	...
	// 表情を変える用の関数
	void SetTexture(int n, Texture2D* tex){ lpTexture[n] = tex; }
};
...
typedef struct tagKEYSET 
{
	...
	u8	START, SELECT;
	//　カメラ用
	u8  ZERO, TWO, FOUR, FIVE, SIX, EIGHT, ENTER;
} KEYSET, *LPKEYSET;
